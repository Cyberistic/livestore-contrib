...
export const makePersistedAdapter =
  ...
  (adapterArgs) =>
    Effect.gen(function* () {
      ...
      yield* shutdownChannel.listen.pipe(
        Stream.tap((result) => {
          if (Result.isSuccess(result)) {
            const cause = result.success;
            return shutdown(
              cause._tag === "IntentionalShutdownCause" ? Exit.succeed(cause) : Exit.fail(cause),
            );
          }
          return Effect.void;
        }),
      ...
      const devtoolsUrl =
        devtoolsEnabled === true ? getDevtoolsUrl().toString() : "ws://127.0.0.1:4242";
...
const makeLeaderThread = ({
  ...
  Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      yield* Queue.takeBetween(bootStatusQueue, 1, 1000).pipe(
...
const resolveExpoPersistencePaths = ({
  ...
  const subDirectory =
    storage?.subDirectory !== undefined ? `${storage.subDirectory.replace(/\/$/, "")}/` : "";
  ...
  Effect.sync(() => {
    ...
    return {
      ...
      boot: Effect.gen(function* () {
        ...
        const node = yield* Webmesh.makeMeshNode(
          Devtools.makeNodeName.client.leader({ storeId, clientId }),
        );
